// Common types for the Survey Platform application
...
// Survey Platform specific types
export type UserRoleType = 'admin' | 'business' | 'user';
export type BusinessSize = 'startup' | 'small' | 'medium' | 'large' | 'enterprise';
...
export interface Survey {
  ...
  business_id: string;
  template_id?: string;
  survey_data: any; // JSONB data containing questions
  settings: any; // JSONB data containing survey settings
  status: SurveyStatus;
  qr_code_url?: string;
  share_url?: string;
...
export interface SurveyResponse {
  id: string;
  survey_id: string;
  respondent_id?: string; // null for anonymous
  response_data: any; // JSONB data containing answers
  metadata: any; // JSONB data containing IP, user agent, etc.
...
export interface SurveyQuestion {
  id: string;
  type: 'text' | 'textarea' | 'multiple_choice' | 'checkbox' | 'rating' | 'slider' | 'date' | 'file';
  title: string;
  description?: string;
  required: boolean;
  options?: string[]; // For choice questions
  scale?: number; // For rating questions (1-5, 1-10, etc.)
